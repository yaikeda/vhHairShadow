...
void AkgRibbonHairBall::Create(t_ObjectInfo* objectInfo)
{
  float uv[8][2] = {
    1.0, 1.0,
    1.0, 0.0,
  ...
  //ribbon
  std::vector<float> RibbonVert;
  for (int i = 0; i < LineVert.size(); i+=3) {
    //同じ座標の点を二点ずつ登録
    RibbonVert.push_back(LineVert[i]);
    RibbonVert.push_back(LineVert[i+1]);
    RibbonVert.push_back(LineVert[i+2]);
  ...
  glBufferData(GL_ARRAY_BUFFER, RibbonVert.size() * sizeof(float), &RibbonVert[0], GL_STATIC_DRAW);
...
void AkgRibbonHairBall::GetBoundingBox(std::vector<float>* vert)
{
