...
AkgRenderOpacityEvaluate::AkgRenderOpacityEvaluate(void)
{}

AkgRenderOpacityEvaluate::~AkgRenderOpacityEvaluate(void)
{}

void AkgRenderOpacityEvaluate::Render(
  GLuint program, 
  ...
  t_CameraInfo cameraInfo)
{
  ...
  /*
    ...
    if (strcmp(objectInfo[i].name, "HairBall") != 0) {
      location = glGetUniformLocation(program, "uModelMtx");
      if (location >= 0) {
      ...
      //error
      //ヘアボールならmodel matrix送らない
      //すでに，ulightMVPが含んでしまっている．
      glm::mat4 dummyMatrix = glm::mat4(1.0f);
      location = glGetUniformLocation(program, "uModelMtx");
      if (location >= 0) {
        glUniformMatrix4fv(location, 1, GL_FALSE, &dummyMatrix[0][0]);
      } else {
	      fprintf(stderr, "ユニフォーム変数%sが見つからない\n", "uModelMtx");
      }
    }
