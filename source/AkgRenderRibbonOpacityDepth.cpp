...
AkgRenderRibbonOpacityDepth::AkgRenderRibbonOpacityDepth(void)
{}

AkgRenderRibbonOpacityDepth::~AkgRenderRibbonOpacityDepth(void)
{}

void AkgRenderRibbonOpacityDepth::Render(
  GLuint program,
  ...
  )
{
  if (objectInfo.size() >= 2) {
    std::cout << "RibbonHairBallのデータを使用します" << std::endl;
  }
  ...
  glm::mat4 projMtx = glm::perspective<float>(80, 1.0f, nearFar[0]-10, nearFar[1]);
...
//protected
void AkgRenderRibbonOpacityDepth::GetClippingFromLight(
  float nearFar[],
  glm::mat4 modelMtx,
  glm::mat4 viewMtx,
  std::vector<t_ObjectInfo> objectInfo)
